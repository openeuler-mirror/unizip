...
int zip7_deflate2(struct zip7_ctx *ctx)
{
    ...
    SizeT outlen = zip7_compressBound(ctx->srcLen);
    ...
    ret = LzmaEncode((Byte *)(ctx->dest + LZMA_PROPS_SIZE), &outlen,
...
int zip7_inflate(struct zip7_ctx *ctx)
{
    if (ctx->total_in == 0) {
        ctx->src = ctx->next_in;
        ctx->dest = ctx->next_out;
        SRes res = LzmaDec_Allocate(&ctx->dec, (Byte *)ctx->src,
    ...
    LzmaDec_DecodeToBuf(&ctx->dec, (Byte *)ctx->dest, &ctx->destLen,
                        (Byte *)ctx->src, &ctx->srcLen, LZMA_FINISH_ANY,
                        &ctx->status);
